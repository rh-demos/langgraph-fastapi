...
INFERENCE_MODEL = os.getenv("INFERENCE_MODEL", "vllm/qwen3-8b")
...
logger.info("  toolgroups: %s", list(agent_config.toolgroups))
...
def run_agent_turn(user_message: str) -> dict:
...
def run_agent_turn_streaming(user_message: str):
...
def agent_node(state: AgentState) -> dict:
    ...
    logger.info("  [Agent] Processing: %s", state["query"][:80])
    result = run_agent_turn(state["query"])
...
def read_root():
    return {
        ...
        "endpoints": {
            "find_orders": "/find_orders?email=<customer_email>  (SSE stream)",
            "find_invoices": "/find_invoices?email=<customer_email>  (SSE stream)",
            "question": "/question?q=<your_question>  (SSE stream)",
        },
        "features": {
            "toolgroups": ["mcp::customer", "mcp::finance", "builtin::websearch"],
            "input_shields": INPUT_SHIELDS,
...
async def ask_question(q: str):
    ...
    logger.info("=" * 80)
    logger.info("API: Processing question (stream): %s", q)
    logger.info("=" * 80)
    ...
    async def event_generator():
        try:
            ...
            has_content = False
            for event in run_agent_turn_streaming(q):
